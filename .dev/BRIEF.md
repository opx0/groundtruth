...
| EPA AQS | Historical PM2.5 and ozone monitor summaries | `aqs.epa.gov/data/api/monitors/byBox`, `annualData/byBox`, `dailyData/byBox`; params `88101` PM2.5, `44201` ozone | Free key by email | Docs verified. 10 requests per minute, 5-second pause requested. Begin and end dates must fall in one year except for `monitors`. Data can lag 6 months or more. The shared test account is capped daily and was exhausted on 2026-09-15 and again on 2026-09-16; an operator registered a key of their own on 2026-09-16 and both the success and the empty answer are recorded. **The envelope is `Data`, not `Body` — see the correction in the endpoint table at the end.** |
| AirNow | Current preliminary conditions | `airnowapi.org/aq/observation/latLong/current` | Free key | Returns 401 without a key. Observations update hourly. Rate limits are documented behind login. ~~*Response shape unverified until a key exists.*~~ **A key exists as of 2026-09-16 and the shape is recorded** — `tests/fixtures/airnow/current-observations-houston.json`. All four derived field names were right. The per-service documentation is still behind a login. |
...
| AirNow without a key, 2026-09-16 | HTTP 401, body `{"WebServiceError":[{"Message":"Request not authenticated."}]}`. So its error envelope is `WebServiceError`, an array of `{Message}`. Recorded as `tests/fixtures/airnow/unauthenticated.json`. ~~Its success shape is still unverified.~~ **No longer true — see the row below.** |
| AirNow with a key, 2026-09-16 | Three rows for the demonstration coordinate — O3, PM2.5 and PM10, reporting area `Houston-Galveston-Brazoria`. Recorded as `tests/fixtures/airnow/current-observations-houston.json`; the empty answer is `tests/fixtures/airnow/no-observations.json`, a bare `[]`. All four field names `lib/adapters/airnow.ts` derived came back spelled exactly as derived. `Category` arrives as an object, `{"Number":1,"Name":"Good"}`, which is why it was right to leave out of the schema rather than guess at. |
| AQS response envelope, from EPA's own published API documentation | ~~`{"Header":[{status,request_time,url,rows}],"Body":[...]}`. It is `Body`, not `Data`.~~ **Wrong. Corrected 2026-09-16 by the first real response.** It is `Data`. The documentation page does print `Body`, in a worked example for `sampleData` — a different service of the same API — and EPA's OpenAPI file said `Data` all along. The page was believed over the specification because the page carried actual JSON; the reasoning was sound and the conclusion was wrong. |
| AQS with a key, 2026-09-16 | `annualData/byBox` at the demonstration coordinate for 2025: 212 rows, thirty distinct monitors, twenty-nine inside B2's 50 km circle. Recorded as `tests/fixtures/aqs/annual-summary-houston.json`, with the two credentials in EPA's echoed `Header[0].url` replaced and nothing else edited; the empty answer is `tests/fixtures/aqs/annual-summary-no-rows.json` (`"No data matched your selection"`, `rows: 0`). Two column names besides the envelope were wrong: `parameter`, not `parameter_name`; `units_of_measure`, not `unit_of_measure`. Both came from the same `sampleData` worked example. EPA still publishes no column list for `annualData`. |
