 ...
 * separated on screen. The monitor asserted below is 36.11 km from the mapped
 * point -- the furthest PM2.5 monitor EPA returned inside the 50 km box -- and
 * is in the part of the list the reader has to open, the nearest one being
 * 1.52 km away, so this test drives the "Show the rest" seam as well.
 */
...
test("a distant monitor", async ({ page }) => {
	...
	const nearest = aqs.locator("[data-record='48-201-1035-88101']");
	await expect(nearest).toContainText(
		"PM2.5 monitor 48-201-1035-88101 is 1.52 km from the mapped point, and measures its own location, not this address.",
	);

	// The distant one is in the rest of that listing.
	await expect(aqs.locator("[data-record='48-201-0066-88101']")).toHaveCount(0);
	await aqs.locator("[data-toggle='aqs:0']").click();

	const distant = aqs.locator("[data-record='48-201-0066-88101']");
	await expect(distant).toContainText(
		"PM2.5 monitor 48-201-0066-88101 is 36.11 km from the mapped point, and measures its own location, not this address.",
	);
	...
	await expect(distant.locator("[data-field='distanceMeters']")).toHaveText("36.11 km");
	await expect(distant.locator("[data-field='value']")).toHaveText("9.339437");
	await expect(distant.locator("[data-field='unit']")).toHaveText("Micrograms/cubic meter (LC)");
	await expect(distant).toContainText("2025 annual arithmetic mean: 9.339437 Micrograms/cubic meter (LC).");
	...
	await expect(panel.locator("[data-row='record-ids']")).toContainText("48-201-0066-88101");
	const clicked = panel.locator("[data-row='value'][data-presence='clicked']");
	await expect(clicked).toContainText("36.11 km");
